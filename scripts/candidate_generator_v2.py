...
def parser():
    ...
    for k in ('graph','probability','output'):p.add_argument('--'+k,type=Path,required=True)
...
def run(c):
    ...
    start=time.perf_counter();g=load_graph(c.graph);p=probability(c.probability,g.graph['inference_shape']);ctx=Context(g,p,c.tangent_lookback);t=time.perf_counter();local,audit=generate_local(ctx,c);localtime=time.perf_counter()-t;t=time.perf_counter();corridors,search=generate_corridors(ctx,c);corridortime=time.perf_counter()-t
    ...
    write_json(c.output/'provenance.json',{'coordinate_order':'x,y','units':'inference_pixel','inference_shape':list(p.shape),'graph_sha256':hashlib.sha256(c.graph.read_bytes()).hexdigest(),'probability_sha256':hashlib.sha256(c.probability.read_bytes()).hexdigest(),'probability_quantization':1/255 if c.probability.suffix=='.png' else None,'status':'proposals_only','graph_mutated':False,'heuristic_is_label':False,'source_sha256':{str(f):hashlib.sha256(f.read_bytes()).hexdigest() for f in [Path(__file__),Path(__file__).parent/'corridor_proposal.py',*sorted(f for f in (Path(__file__).parent/'connection_candidates').glob('*') if f.is_file())]}})

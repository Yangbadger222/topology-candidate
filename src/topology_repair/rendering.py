...
def render_source_views(image_path, graph, source, endpoint_id, candidates, output_dir, local_size=512, context_size=1024):
    ...
    p=graph.nodes[endpoint_id] if endpoint_id is not None else None
    ...
    if p is not None:
        ...
        source_edges=[e for e in graph.edges if e.edge_id in source.source_edge_ids]
        coords=[pt for e in source_edges for pt in e.coordinates]
        if not coords: coords=[(im.width/2, im.height/2)]
        xs,ys=zip(*coords); pts=[((min(xs)+max(xs))/2,(min(ys)+max(ys))/2)]
    ...
    for kind,size in (("local",local_size),("context",context_size)):
        ...
        for name,img in ((f"raw_{kind}",raw),(f"overlay_{kind}",overlay)):
            suffix = f"ep_{endpoint_id}" if endpoint_id is not None else "source_only"
            path=output_dir/f"{source.source_subgraph_id}_{suffix}_{name}.png"; img.save(path); result[name]=str(path)
    return result

...
SCHEMA_VERSION=2
REASON_CODES=["ROOFTOP","BUILDING_EDGE","SHADOW","VEGETATION","TRUE_DEAD_END","PARALLEL_ROAD","WRONG_TARGET","OCCLUDED_ROAD","AMBIGUOUS_IMAGE","CORRECT_TARGET_MISSING","OTHER"]
...
def new_annotation(scene_id, source, annotator="human", provenance=None):
    ...
    return {"schema_version":SCHEMA_VERSION,"scene_id":scene_id,"source_subgraph_id":source["source_subgraph_id"],
            "source_type":source["source_type"],"component_validity":"UNCERTAIN","endpoint_labels":{},
...
def update_endpoint(annotation, endpoint_id, selection, reason_codes=None, notes="", candidate_ids=None):
    ...
    annotation["endpoint_labels"][str(endpoint_id)]={"selection":selection,"derived_action":derive_action(annotation["component_validity"],selection),
